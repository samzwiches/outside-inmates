...
export default async function ChildrenPage() {
  ...
  return <FamilyGuideLayout slug="children"><div className="guide-reading"><section><h2>What to say at different ages.</h2><p>Children need honesty they can understand, not a perfect speech. These are adaptable examples, not scripts families must follow.</p><div className="age-guidance-grid">{childAgeGuidance.map((item, index) => {
    const cardKey = `families.children.age.${String(index + 1).padStart(2, "0")}`;
    const card = cardsByKey.get(cardKey);
    return <article className={`${card?.imageUrl ? "has-card-image" : ""} ${card?.tone ? `card-tone-${card.tone}` : ""}`} data-card-key={cardKey} key={item.label}>{card?.imageUrl ? <SiteCardImage src={card.imageUrl} alt={card.imageAlt} focalX={card.focalX} focalY={card.focalY} /> : null}<h3>{card?.title ?? item.label}</h3><p>{card?.description ?? item.example}</p></article>;
