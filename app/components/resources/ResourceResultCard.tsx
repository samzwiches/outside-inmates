...
export function ResourceResultCard({ resource }: { resource: ResourceData }) {
  return (
    <article className="resource-result-card">
      ...
      <h2><a href={`/resources/${resource.slug}`}>{resource.name}</a></h2>
      ...
      <div className="result-actions">
        ...
        <a className="button button-secondary" href={`/resources/${resource.slug}`}>View details <span aria-hidden="true">→</span></a>
      </div>
    </article>
  );
}
