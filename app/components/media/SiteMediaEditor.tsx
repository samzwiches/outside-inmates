...
export function SiteMediaEditor({ media, appearances, supabaseConfig }: EditorProps) {
  ...
  async function validateClientImage(file: File) {
    ...
    if (!((file.type === "image/jpeg" && jpeg) || (file.type === "image/png" && png) || (file.type === "image/webp" && webp))) return "The selected file does not contain a valid image signature.";
    return null;
  }

  async function setImageFile(file: File | null, target: UploadRole) {
    if (!file) return;
    const error = await validateClientImage(file);
    if (error) {
  ...
  async function uploadImageDirectly(file: File, role: UploadRole): Promise<string> {
    ...
    const preparation = await fetch("/api/admin/site-media/upload-url", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ mediaKey: selectedKey, role, name: file.name, type: file.type, size: file.size }),
    });
    const preparationPayload = await preparation.json().catch(() => null);
    if (!preparation.ok) throw new Error(recordError(preparationPayload, "The image upload could not be prepared."));
    const upload = preparationPayload?.upload as PreparedUpload | undefined;
  ...
  async function saveMedia(event: React.FormEvent) {
    ...
    try {
      [primaryPath, mobilePath] = await Promise.all([
        primaryFile ? uploadImageDirectly(primaryFile, "primary") : Promise.resolve(null),
        mobileFile ? uploadImageDirectly(mobileFile, "mobile") : Promise.resolve(null),
      ]);
    } catch (error) {
      setMediaStatus({ kind: "error", message: error instanceof Error ? error.message : "The image could not be uploaded. Please try again." });
      return;
    }
    ...
    if (primaryPath) formData.set("primaryPath", primaryPath);
    if (mobilePath) formData.set("mobilePath", mobilePath);
