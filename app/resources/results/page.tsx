...
export default async function ResourceResultsPage({ searchParams }: { searchParams: Promise<ResourceSearchParams> }) {
  ...
  const emptyVariant = !sampleResources.length ? "unavailable" : !hasCriteria ? "no-location" : hasNarrowingFilters(filters) ? "narrow" : "no-results";
      ...
      <main id="main-content" className="results-page">
        <section className="results-search-header">
          <div className="container">
            ...
            <p>Every result below is demonstration data for the directory experience. Use the search and filters to see how it will work.</p>
            <ResourceSearchPanel initial={filters} className="results-search-panel" />
          </div>
        </section>
